...
The file *Case_study_1_computational_results_KW51_Mode06.qmd* replicates all computations of the KW51 case study in Section 4 of the paper.
...
The file *Case_study_1_plot_results_KW51_Mode06.qmd* replicates all KW51 case study related plots in Section 4 of the paper.
...
* Output:
  ...
  - Figure 3 (Artificial data exemplary response, covariate and error profiles)
...
The file *SHEWHART_ARL_calculation_daily.R*
is an R-script prepared for parallel computing the run-lengths of the daily Shewhart control chart based on a fit of a piecewise linear model where hourly data (residuals) are averaged to 24h.

* Input: 
  - gam2d_simulation_runlength.RDS (Load the exact same data as for the MEWMA 
  charts from the gam2d_simulation_runlength_XX.RDS files)
* Output: 
  - Run-length files for daily Shewhart charts
  
# Simulation study: monitoring scheme run length - computational results part 3
The file *SHEWHART_ARL_calculation_hourly.R*
is an R-script prepared for parallel computing the run-lengths of the hourly Shewhart control chart based on a fit of a piecewise linear model.
...
The file *Simulations_monitoring_scheme_run_length_plots.qmd* plots ARL profiles for different MEWMA smooting parameters and hourly and daily Shewhart charts.

* Input: 
  - data/simulations/ (Average Run Length profiles for lambda=0.1,0.3,1)
* Output:
  - Figure 5 (MEWMA and Shewhart charts Average Run Length performance comparison)
