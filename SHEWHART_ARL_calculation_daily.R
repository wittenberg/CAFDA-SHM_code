...
SHEWHART_arl_daily <- function(h, hseq, delta, efi, meanU, fb1, sig, modlm) {
  rl <- 0
  T2 <- 0
  while(T2 < h){
    rl <- rl + 1
...
sapply(RUNS, function(ii) {
  ...
  fres    <- dtai |>
              ...
              group_by(ind_dayf) |>
              summarise(meanres24=mean(res)) |>
              summarise(mean24=mean(meanres24), sd24=sd(meanres24))
  sig     <- fres$sd24
  hseq    <- seq(.5, 23.5, 1)
  ...
  parallel::clusterExport(cl, varlist=c("h", "hseq", "delta_vec", "modlm",
                              "meanU", "fb1", "SHEWHART_arl_daily", "resEFI1", "resEFI2", 
  ...
  resEFI1 <- lapply(seq_along(delta_vec), function(j) {
    list(
      parSapplyLB(cl, 1:n, function(i) {
        SHEWHART_arl_daily(h=h,hseq=hseq, delta=delta_vec[j], efi=1, meanU=meanU, 
  ...
  resEFI2 <- lapply(seq_along(delta_vec), function(j) {
    list(
      parSapplyLB(cl, 1:n, function(i) {
        SHEWHART_arl_daily(h=h,hseq=hseq, delta=delta_vec[j], efi=2, meanU=meanU, 
                     fb1=fb1, sig=sig, modlm=modlm)
      })
    )
  })
  ...
  resEFI3 <- lapply(seq_along(delta_vec), function(j) {
    list(
      parSapplyLB(cl, 1:n, function(i) {
        SHEWHART_arl_daily(h=h,hseq=hseq, delta=delta_vec[j], efi=3, meanU=meanU, 
                     fb1=fb1, sig=sig, modlm=modlm)
      })
    )
  })
  ...
  # Save results
  saveRDS(bind_rows(resA, resB, resC, .id="EFI"), paste0(DIR, "SHEWHART_ARL_daily_r_", 1, "_N_300_RUN_", ii, ".rds"))
