...
p1 <- bind_rows(df1, df2, df3, .id="LAMBDA") |>
  ...
  ggplot(aes(x=delta_vec, y=AARL, group=LAMBDA, colour=LAMBDA, fill=LAMBDA)) +
    ...
    theme(legend.position="inside", legend.position.inside=c(.9,.7), legend.title=element_blank())
```

```{r MEWMA_simulation_plots_standardized_shift, fig.width=8.5, fig.height=3}
p2 <- bind_rows(df1, df2, df3, .id="LAMBDA") |>
  mutate(LAMBDA=factor(LAMBDA, labels=LABS), 
...
```{r Figure_5_MEWMA_simulation_plots_combined, fig.width=8.5, fig.height=6}
