...
# Package names
packages <- c("benchmarkme", "dplyr", "forcats", "ggplot2", "gratia", 
...
fig0
save_image(fig0, "Figure_8_compute_compute_model_2_interactive_plot.pdf")
```
...
fig0 <- plot_ly(z=~surface1, y=~as.numeric(rownames(surface1)), 
  ...
  layout(
    ...
    scene=list(
      ...
      yaxis=list(title='Time of day [h]', color="black", tickvals=seq(0,24, 6), autorange="reversed"),
      zaxis=list(title="Intercept", color="black", range=c(-.007, .007), nticks=5),
    camera=list(
      margin=list(l=0, r=0, b=0, t=0),
...
fig0
save_image(fig0, "Figure_9a_plot_2d_3D_fig1_mode06.pdf")
```
...
```{r Figure_9b_plot_gam6, fig.width=8, fig.height=4}
p2 + p3 + plot_layout(ncol=2)
```

## Model 4 (KW51 extended_model_interactions)

```{r Figure_10a_compute_7d_3D_fig2, dev='svg'}
...
fig2 <- plot_ly(z=~surface2, y=~as.numeric(rownames(surface2)), 
...
save_image(fig2, "Figure_10a_compute_7d_3D_fig2.pdf")
```

```{r Figure_10b_compute_7d_3D_fig1, dev='svg'}
...
fig1
save_image(fig1, "Figure_10b_compute_7d_3D_fig1.pdf")
```
...
### MEWMA plot interactions lambda = 0.3
...
nu     <- VarCorr(gam2d$lme)[c("ef01", "ef02", "ef03", "ef04"),][,"Variance"] |>
  t() |>
  as.data.frame() |>
  as.numeric()
s2     <- diag(nu)
...
res1 <- data.frame(day=1:472, T2=T2a) |>
  mutate(date=seq.POSIXt(first(DFa$date), last(DFa$date), by="day"))
```

```{r MEWMA_chart_without_retrofitting_interactions_lambda_03}
## omit days with 24 NA's per day
NAgeq24 <- DFa |>
  select(mode, temp, humid, ind_day, ind_hour) |>
  group_by(ind_day) |>
...
DF <- DFb |> filter(!ind_day %in% NAgeq24$ind_day) 
...
for(i in z) {
  ...
  T2b[i] <- t(Z-mu0) %*% invSig %*% (Z-mu0)
}

## create data frame for plotting
res2 <- data.frame(day=1:472, T2=T2b) |>
  mutate(date=seq.POSIXt(first(DFb$date), last(DFb$date), by="day"))
```

```{r combine_data_and_rects_for_retrofitting_phase_interactions_lambda_03}
## combine data with and without retrofitting for plotting
res3 <- bind_rows(res1, res2, .id="CHART")
## rectangle retrofitting phase for plotting
rects <- data.frame(
...
```{r}
filter(res3, T2 > h4, CHART==1) |> head(n=10)
filter(res3, T2 > h4, CHART==2) |> head(n=10)
```
...
```{r MEWMA_chart_without_retrofitting_interactions_lambda_1}
## omit days with 24 NA's per day
NAgeq24 <- DFa |>
  select(mode, temp, humid, ind_day, ind_hour) |>
  group_by(ind_day) |>
...
```{r combine_data_and_rects_for_retrofitting_phase_interactions_lambda_1}
## combine data with and without retrofitting for plotting
res4 <- bind_rows(res1, res2, .id="CHART")
## rectangle retrofitting phase for plotting
rects4 <- data.frame(
  xstart=as.POSIXct("2019-05-15", tz="UTC"),
  xend  =as.POSIXct("2019-09-27", tz="UTC"),
  col="grey45", ymin=.1, ymax=1200) |>
  `colnames<-`(c("xstart", "xend", "col", "ymin", "ymax") )
```

```{r}
filter(res4, T2 > h41, CHART==1) |> head(n=10)
filter(res4, T2 > h41, CHART==2) |> head(n=10)
```
#### MEWMA plot

```{r Figure_11_MEWMA_control_charts, fig.width=12, fig.height=7, message=FALSE, warning=FALSE}
## combined plots
lims <- data.frame(
  yintercept=c(h41, h41, h403 , h403),
  CHART_pos=c( 
                             "MEWMA~chart~(lambda==1)~with~retrofitting~phase",
                             "MEWMA~chart~(lambda==1)~without~retrofitting~phase", 
                             "MEWMA~chart~(lambda==0.3)~with~retrofitting~phase", 
                             "MEWMA~chart~(lambda==0.3)~without~retrofitting~phase" 
))

lims <- mutate(lims, CHART_pos=factor(CHART_pos, levels=c( 
                             "MEWMA~chart~(lambda==1)~with~retrofitting~phase",
                             "MEWMA~chart~(lambda==1)~without~retrofitting~phase", 
                             "MEWMA~chart~(lambda==0.3)~with~retrofitting~phase", 
                             "MEWMA~chart~(lambda==0.3)~without~retrofitting~phase" 
...
df <- bind_rows(
  filter(res3, date>="2019-01-01") |>
  mutate(CHART=factor(CHART, labels=c(
    "MEWMA~chart~(lambda==0.3)~with~retrofitting~phase",  
    "MEWMA~chart~(lambda==0.3)~without~retrofitting~phase"
    ))),
  filter(res4, date>="2019-01-01") |>
  mutate(CHART=factor(CHART, labels=c(
    "MEWMA~chart~(lambda==1)~with~retrofitting~phase",  
    "MEWMA~chart~(lambda==1)~without~retrofitting~phase")))
  )|>
  mutate(CHART_pos = factor(CHART, levels=c( 
                             "MEWMA~chart~(lambda==1)~with~retrofitting~phase",
                             "MEWMA~chart~(lambda==1)~without~retrofitting~phase", 
                             "MEWMA~chart~(lambda==0.3)~with~retrofitting~phase", 
                             "MEWMA~chart~(lambda==0.3)~without~retrofitting~phase")))
...
df |>
 ggplot(aes(x=date, y=T2, group=CHART_pos)) +
    geom_point(size=.5) +
    geom_line(linewidth=.3) +
    geom_point(aes(x = date, y = ifelse(T2 > h403, T2, NA), color='red', group=CHART), size=.8) +
    geom_point(data=filter(df,
                           CHART%in%c(
                             "MEWMA~chart~(lambda==1)~with~retrofitting~phase",
                             "MEWMA~chart~(lambda==1)~without~retrofitting~phase")),
               aes(x = date, y = ifelse(T2 > h41, T2, NA), group=CHART), color="red", size=.8) +
    geom_point(data=filter(df, 
                           CHART%in%c(
                             "MEWMA~chart~(lambda==0.3)~with~retrofitting~phase",
                             "MEWMA~chart~(lambda==0.3)~without~retrofitting~phase")), 
               aes(x = date, y = ifelse(T2 > h403, T2, NA), group=CHART), color="red", size=.8) +
    facet_wrap(~CHART_pos, ncol=2, scales="fixed", labeller = label_parsed) +
    geom_hline(data=lims, aes(yintercept=yintercept, group=CHART_pos), color="orange") +
